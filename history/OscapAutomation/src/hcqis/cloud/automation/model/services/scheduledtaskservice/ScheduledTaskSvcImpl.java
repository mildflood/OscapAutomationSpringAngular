...
import hcqis.cloud.automation.model.utility.concurrency.ConcurrentTaskExecuter;
import hcqis.cloud.automation.model.utility.fileutility.PropertyFileReaderUtility;
...
public class ScheduledTaskSvcImpl extends TimerTask implements IScheduledTaskSvc {
	...
	/**
	 * Author Jonas Okwara Call the update method and update the database with new
	 * status message
	 * @param message
	 * @param jobID
	 */
	protected void updateDB(String message, long jobID) {
		ScanManager scanMgr = ScanManager.getInstance();
		scanMgr.updateSchedDB(message, jobID);
	}
